...
const fixture = join(__dirname, "../fixtures/sample.cs");
...
describe("C# extraction", () => {
  test("extracts file node and class", async () => {
    ...
    expect(nodeLabels).toContain("sample.cs"); // file node
    expect(nodeLabels).toContain("Calculator"); // class
  });

  test("extracts import edges from using directives", async () => {
    const result = await extractFile(fixture);
    const importEdges = result.edges.filter((e) => e.relation === "imports");
    expect(importEdges.length).toBeGreaterThanOrEqual(2); // System, Generic
  });

  test("extracts methods", async () => {
    const result = await extractFile(fixture);
    const methodEdges = result.edges.filter((e) => e.relation === "method");
    expect(methodEdges.length).toBeGreaterThanOrEqual(1); // Add, Main
  });
});
